...
const vendor = page.props.vendor as {
    id: number;
    name: string;
    domain: string;
    slug: string;
...
const fetchComboboxData = debounce(() => {
    ...
    router.get(route('vendors.edit', vendor.id), {
...
<template>
    <!-- <pre class="text-xs absolute z-50 bg-red-500">{{ form }}</pre> -->
    <Head title="Edit Vendor" />

    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-4 rounded-xl p-4 overflow-x-auto">
            <div class="flex flex-col space-y-6 min-h-full">
                <HeadingSmall title="Edit vendor" description="Enter the vendor information and select users to grant access" />
